...
def time_sample_tick_to_bar(tick_data : TickData, time_per_bar : timedelta = timedelta(seconds = 1)) -> BarData:
    ...
    rule : str = str(time_per_bar.total_seconds()) + "S"
...
def tick_sample_to_bar(tick_data : TickData, ticks_per_bar : int) -> BarData:
    tmp_df = pd.DataFrame({
    ...
    ts_series = tmp_df.ts.groupby(tmp_df.index // ticks_per_bar).first()
    close_series = tmp_df.p.groupby(tmp_df.index // ticks_per_bar).last()
    volume_series = tmp_df.q.groupby(tmp_df.index // ticks_per_bar).sum()
    tick_series = tmp_df.ts.groupby(tmp_df.index // ticks_per_bar).count()
    return BarData(
